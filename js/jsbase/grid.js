...
Grid.prototype.createTable = function() {
    ...
    for (var j = 0, l = this.data.length; j < l; j++) {
        ...
        for (i in this.colConfig) {
            ...
            /* ------------ Custom Method Field / By Shopping Cart - can be used for custom grid functions ------------ */
            if (typeof col.key === 'undefined' && typeof col.method === 'function' && typeof col.methodDisplay !== 'undefined')
            {
                trcell.innerHTML = col.methodDisplay ;

                /* ------------ Generic Extension Version 3 /w Closure ---------------- */
                (function (ref){
                  trcell.addEventListener('click', function(){col.method(ref)})
                }(this.data[j][col.reference]))
